...
namespace tmap
{
  ...
  enum struct Merger
  {
    EdgeCover =
        0, ///< Based on the matching merger. Computes an edge cover such that
           ...
           ///< The cover edgeList are then used to merge their adjacent vertices.
    LocalBiconnected =
  ...
  /**
   * @brief Genereates coordinates, edgeList and properties of a MST (via a kNN
   ...
   * @param edgeList An edge list in the form of [(from, to, weight)].
   * @return std::tuple<std::vector<uint32_t>, std::vector<uint32_t>, std::vector<float>>
   */
  std::tuple<std::vector<uint32_t>, std::vector<uint32_t>, std::vector<float>>
  MSTFromEdgeList(uint32_t vertex_count, const std::vector<std::tuple<uint32_t, uint32_t, float>> &edgeList);

  /**
   * @brief Genereates coordinates, edgeList and properties of a MST from an edge
   * list.
   *
   * @param vertex_count The number of vertices in the input graph.
   * @param edgeList An edge list in the form of [(from, to, weight)].
  ...
  LayoutFromEdgeList(
      ...
      const std::vector<std::tuple<uint32_t, uint32_t, float>> &edgeList,
      LayoutConfiguration config = LayoutConfiguration(),
      bool keep_knn = false,
      bool create_mst = true);
